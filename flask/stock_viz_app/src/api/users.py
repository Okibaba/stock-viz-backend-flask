...
def create():
    ...
    # Check the lengths of the username and password
...
def delete(id: int):
    u = User.query.get_or_404(id, "Username not found")
    try:
...
def update(id: int):
    ...
    # Check if username and password are both not provided
    ...
    try:
        db.session.commit()  # Saving the updates
        # Return the updated user serialized and jsonified
        return jsonify(u.serialize())
    except:
        # something went wrong :(
        return jsonify(False)
